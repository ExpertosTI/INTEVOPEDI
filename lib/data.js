...
import { isValidCertificateCode, normalizeCertificateCode, normalizeEmail, sanitizeText } from '@/lib/validation';
...
export async function getParticipantAccessEnrollment(referenceCode, email) {
  try {
    const enrollment = await prisma.enrollment.findUnique({
      where: { referenceCode: sanitizeText(referenceCode).toUpperCase() },
      include: {
        participant: true,
        course: true,
        certificate: true
    ...
    if (!enrollment || enrollment.participant.email !== normalizeEmail(email)) {
      return null;
    }
