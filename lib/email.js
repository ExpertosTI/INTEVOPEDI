...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: process.env.SMTP_PASSWORD || process.env.SMTP_PASS || 'JustWork2027@',
  },
  tls: {
...
export async function sendEmail({ to, subject, text, html }) {
  try {
    const fromAddress = process.env.SMTP_FROM || process.env.EMAIL_FROM || '"Intevopedi" <info@renace.space>';
